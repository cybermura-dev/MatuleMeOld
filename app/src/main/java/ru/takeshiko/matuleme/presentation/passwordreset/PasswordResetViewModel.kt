...
class PasswordResetViewModel(
    ...
    fun verifyUser(email: String, secret: String) {
        viewModelScope.launch {
            try {
                when (val result = authRepository.confirmPasswordReset(email, secret)) {
                    is AuthResult.Success -> _verifyResult.value = AuthResult.Success(result)
                    is AuthResult.Error -> _verifyResult.value = AuthResult.Error(result.message)
                }
            } catch (e: Exception) {
                _verifyResult.value = AuthResult.Error(e.message ?: "Password reset failed!")
            }
        }
    }
    ...
    suspend fun resendSecret(email: String): AuthResult<Any> {
        return try {
